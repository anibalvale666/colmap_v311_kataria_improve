    ...
    for (auto const &im2 : image_names) {
      if (im1.compare(im2) == 0) {
        ...
        for (correspondence_itr = correspondences.begin();
             ...
             ++correspondence_itr) {
          CorrespondenceGraph::CorrespondenceRange range = sg.FindCorrespondences(im1_.ImageId(), (*correspondence_itr).point2D_idx1);
          std::vector<CorrespondenceGraph::Correspondence>
              point_correspondences;

          for (const CorrespondenceGraph::Correspondence* corr = range.beg; corr != range.end; ++corr) {
            point_correspondences.push_back(*corr);
          }
...
void MDS::SetRegisteredImages(const std::set<image_t>& reg_image_ids) {
